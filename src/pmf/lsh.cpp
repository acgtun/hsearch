...
KLSH::KLSH(const uint32_t& dimension, const uint32_t& hash_bit_num,
           const double& sigma)
    : m_dimension(dimension),
      ...
      m_project_w(m_hash_bit_num, vector<double>(m_dimension)),
  ...
  for (uint32_t i = 0; i < m_hash_bit_num; ++i) {
    ...
    // init project_w (m_dimension + 1, 1 more space for index -1)
