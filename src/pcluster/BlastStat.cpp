...
void BlastStat::BlastGapDecayCorr(double *e, int nsegs) {
  double gap_decay_divisor = (1.0 - gap_decay_rate)
      * pow(gap_decay_rate, nsegs - 1);
  *e /= gap_decay_divisor;
}
...
double BlastStat::sumScore(int tot, double *score, int subjctlen) {
  ...
  double n_score = L * totalscore - lgkmn - (tot - 1) * (LogK + 2 * LogG)
      - log(fac(tot));
...
double BlastStat::sumScore2Expect(int tot, double sumS, int subjctlen) {
  ...
  double sumP_corrected = sumP
      / (pow(DEFAULT_GAP_DECAY, tot - 1) * (1 - DEFAULT_GAP_DECAY));
...
int BlastStat::blastComputeLengthAdjustmentSimple(int query_length,
                                                  int db_length,
                                                  int db_num_seqs) {
...
int BlastStat::blastComputeLengthAdjustment(int query_length,
                                            int *length_adjustment) {
